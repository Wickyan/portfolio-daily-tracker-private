...
- 对买入/新增，如果同一标的出现在多个账户，不要自动沿用账户。
- 对卖出，如果该标的只在一个账户有现有持仓，自动选择该账户并明确提示推断依据。
- 对卖出，如果多个账户都有该标的，列出账户并要求用户选择。
- 当前不支持卖空：没有现有持仓或卖出数量超过持仓数量时，禁止确认写入。
...
- “银河增加2w/银河减少1.5k”在没有股票数量单位时按银河账户CNY现金的deposit/withdraw解析。
- 买入数量单位支持“股/份/个”；例如“小米 尊嘉买入100个 20”应解析为尊嘉账户、小米集团1810、100股、成本价20HKD。
