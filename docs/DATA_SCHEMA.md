...
- position身份`account+code+currency`不得重复，且三项都不能为空。
- cash身份`account+currency`不得重复，且两项都不能为空。
- 持仓数量必须大于0；成本价必须为有限且非负的数字。
- 现金余额必须为有限且非负的数字；增减金额必须大于0。
- 当前可计入实时总资产的币种为`CNY`、`USD`、`HKD`，其他币种写入会被阻止。
- 行情源返回0、负数、NaN或Infinity时，不覆盖最近一次有效价格。
- 账本解析失败或检测到重复身份时，服务拒绝写入，而不是降级为空账本。
...
因此首笔买入手续费不会被漏掉，后续加仓时也按相同规则计入加权成本。代码和币种必须匹配当前行情适配口径：4位数字代码按HKD、6位数字代码按CNY、英文字母代码按USD处理；冲突记录不会写入账本。
